...
import doctorGroups from "./doctorGroups.vue"
export default {
    props:{
    ...
    methods:{
        ...
        // 获取合作达人列表
        getCooperateLiveAnchorClick(){
            api.getCooperateLiveAnchor().then((res)=>{
                if(res.code == 0){
                    // this.performance =  res.data.performance
                    const {CooperateLiveAnchorList} = res.data
                    this.doctorList =  CooperateLiveAnchorList
                    this.doctorName = CooperateLiveAnchorList ? CooperateLiveAnchorList.find(item=>item.name == '小羽毛').name : ''
                    this.doctorId = CooperateLiveAnchorList ?CooperateLiveAnchorList.find(item=>item.name == '小羽毛').id : ''
                    let name=[]
                    CooperateLiveAnchorList.map(item=>{
                        name.push(item.name)
                    })
    ...
    created(){
        this.getAnchor()
        this.isLoading = true
        this.getCooperateLiveAnchorClick()
        setTimeout(()=>{
            this.getPerformanceByLiveAnchorName()
        },1000)
